...
// Below this lux value the room is considered "dark" and the app switches
// to dark theme while in Auto mode. Typical reference points: direct
// sunlight ~10,000+ lux, a lit room ~100-300 lux, dim room ~10-50 lux, a
// hand fully covering the sensor ~0-2 lux.
const double _kDarkLuxThreshold = 10;

final themeViewModelProvider = NotifierProvider<ThemeViewModel, ThemeState>(ThemeViewModel.new);
...
class ThemeViewModel extends Notifier<ThemeState> {
  ...
  Future<void> _startListeningToSensor() async {
    try {
      ...
      if (!hasSensor) {
        // No ambient light sensor on this device — just stay on
